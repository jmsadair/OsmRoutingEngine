...
bidirectional_search::bidirectional_search(const std::unordered_map<uint64_t, vertex>*vertices, const std::unordered_map<uint64_t, std::unordered_map<uint64_t, uint64_t>>*shortcuts)
        : vertices_(vertices), shortcuts_(shortcuts), queue_(100)
{}

std::pair<std::vector<uint64_t>, double> bidirectional_search::execute_search(const uint64_t source, const uint64_t target) {
    ...
    while (!queue_.is_empty()) {
        ...
        */
        if (visited_source_.find(u) != visited_source_.end() && visited_target_.find(u) != visited_target_.end()
            ...
            if (best <= queue_.peek_value()) { break; }
        }
    }
...
std::pair<std::vector<uint64_t>, double> bidirectional_search::execute_standard_search(const uint64_t source, const uint64_t target) {
    // u is the vertex being settled and intersection is the vertex at which the forward and backwards search meet.
    uint64_t u, intersection = 0;
    // length of the shortest path found so far.
    double best = INF_;
...
void bidirectional_search::relax_edge(const uint64_t vertex_id, const bool backward, const bool standard) {
...
std::vector<uint64_t> bidirectional_search::reconstruct_path(const uint64_t source, const uint64_t target, const uint64_t intersection) {
...
std::vector<uint64_t> bidirectional_search::unpack_shortcut(uint64_t start, uint64_t end) {
...
std::vector<uint64_t> bidirectional_search::unpack_path(const std::vector<uint64_t>* shortest_path) {
    int i = 0;
...
bool bidirectional_search::is_shortcut(const uint64_t start, const uint64_t end) {
