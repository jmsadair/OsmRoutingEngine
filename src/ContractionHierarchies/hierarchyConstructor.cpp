...
constructor::constructor(graph& graph) :graph_(graph), total_edges_added_(0) { vertices_ = graph_.get_vertices(); }
...
int constructor::contract_vertex(const uint64_t contracted_vertex, const int hop_limit, const bool simulated) {
...
std::unordered_map<uint64_t, double> constructor::witness_search(const uint64_t source, const uint64_t contracted_vertex, const double max_distance, const int hop_limit) {
    queue queue;
    ...
    while (!queue.is_empty() && targets_seen < targets.size() && queue.peek_value() <= max_distance && hops < hop_limit) {
        ...
        if (targets.find(u) != targets.end()) { targets_seen++; }
...
uint64_t constructor::get_next(const int hop_limit, queue* queue) {
    ...
    const uint64_t contracted_vertex = queue->pop();
    return contracted_vertex;
}

void constructor::add_shortcuts(const uint64_t contracted_vertex, const std::vector<std::tuple<uint64_t, uint64_t, double>>* shortcuts_needed) {
...
void constructor::contracted_neighbors(const uint64_t contracted_vertex) {
...
double constructor::get_max_out_distance(const uint64_t contracted_vertex) const {
    double max_out = 0.0;
...
int constructor::get_edge_difference(const uint64_t contracted_vertex, const int hop_limit) {
...
int constructor::get_priority_term(const uint64_t contracted_vertex, const int hop_limit, const bool simulated) {
    if (simulated) {
